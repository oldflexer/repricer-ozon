...
def get_cached_products():
    """Возвращает список товаров (объекты ProductInfo). Используем cache_resource."""
    return repo.get_all_products()

@st.cache_data(ttl=3600, show_spinner=False)
def get_cached_kpi():
    """KPI – словарь, можно кэшировать через cache_data."""
    return repo.get_kpi_metrics()

@st.cache_data(ttl=3600, show_spinner=False)
def get_cached_strategy_roi():
    """DataFrame, cache_data подходит."""
    return repo.get_strategy_roi()

@st.cache_data(ttl=3600, show_spinner=False)
def get_cached_ozon_price_df():
    """DataFrame, cache_data подходит."""
    return repo.get_ozon_index_vs_price()

@st.cache_data(ttl=3600, show_spinner=False)
def get_cached_last_prices():
    """DataFrame, cache_data подходит (требует метод get_all_last_prices в db.py)."""
    return repo.get_all_last_prices()

# --------------------------------------------------------------
# Основные функции репрайсинга
# --------------------------------------------------------------
...
# --------------------------------------------------------------
# Функция для отрисовки боковой панели (без дублирования)
# --------------------------------------------------------------
...
def render_statistics_tab():
    ...
    # Используем оптимизированный запрос (если метод есть, иначе падаем на старый код)
    try:
        ...
        # fallback на старый код (если метод ещё не добавлен в db.py)
        products = get_cached_products()
        if not products:
            st.warning("Нет данных для статистики.")
            return
