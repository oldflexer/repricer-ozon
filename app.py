...
st.title("🔄 Репрайсер Ozon (API only)")
    ...
    if uploaded_file is not None:
        ...
        if st.button("Запустить с новым файлом", use_container_width=True):
            with st.spinner("Полный цикл с новым файлом..."):
                try:
                    stats = run_repricing(dry_run=False)
                    st.success(f"✅ Готово! Обновлено цен: {stats.get('prices_updated', 0)}")
                except Exception as e:
                    st.error(f"❌ Ошибка: {e}")
...
tab1, tab2, tab3, tab4 = st.tabs([
    "📦 Товары", "📈 История", "📊 Графики", "📊 Статистика"
])
        ...
        if selected:
            ...
            for label in selected:
                ...
                if hist:
                    ...
                    # Округляем цены для отображения на графике
...
with tab4:
    ...
    if not products:
        ...
        col2.metric("Средняя цена", f"{avg_price:.0f} ₽")  # округлено до целых
