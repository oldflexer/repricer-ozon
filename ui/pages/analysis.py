...
def render_commissions_analysis():
    repo = get_repo()
...
def render_abc_analysis():
    ...
    # Категории: A (0-80%), B (80-95%), C (95-100%)
    def assign_category(percent):
        if percent <= 80:
            return 'A'
        elif percent <= 95:
            return 'B'
    ...
    category_stats = df_sorted.groupby('category').agg(
    ...
    with col2:
        fig_pie = px.pie(category_stats, values='количество', names=category_stats.index,
    ...
    fig_pareto.update_layout(
        ...
        xaxis_title='SKU',
        yaxis_title='Прибыль (₽)',
        yaxis2=dict(title='Накопленный процент (%)', overlaying='y', side='right', range=[0, 100]),
    ...
    st.dataframe(df_sorted[['sku', 'product_name', 'profit_positive', 'cumulative_percent', 'category']].rename(
        columns={'profit_positive': 'Прибыль (₽)', 'cumulative_percent': 'Накопленный %', 'category': 'Категория'}
    ), width="stretch", hide_index=True)

    st.caption("ABC-анализ проведён по вкладу в общую прибыль (реальная цена - себестоимость). Категории: A — 80% прибыли, B — следующие 15%, C — остальные 5%.")