"""
Страница «Запросы / Управление товарами» дашборда.

Позволяет просматривать информацию о товаре, историю цен,
график динамики и удалять товар из БД.
"""
...
def render_requests_page() -> None:
    """
    Рендерит страницу управления товарами.
    """
    st.markdown(
        '<h2><i class="fa-solid fa-clipboard-list"></i> Запросы / Управление товарами</h2>',
    ...
    if selected_sku:
        ...
        if product:
            ...
            if hist:
                ...
                st.dataframe(
                    df_hist[["timestamp", "customer_price", "marginality_pct"]].rename(
                        columns={
                            "timestamp": "Дата",
                            "customer_price": "Цена (₽)",
                            "marginality_pct": "Маржинальность (%)",
                        }
                    ),
                    width="stretch",
                    hide_index=True,
            ...
            st.warning(
                "Удаление товара приведёт к удалению всех связанных записей "
                "(стратегии, история цен, история маржинальности). Действие необратимо."
            )
            if st.button(
                f"Удалить товар {product.sku}",
                type="secondary",
                ...
                if result["product"] > 0:
                    st.success(
                        f"Товар {product.sku} удалён. Удалено записей: "
                        f"product={result['product']}, strategies={result['strategies']}, "
                        f"price_history={result['price_history']}, "
                        f"margin_history={result['margin_history']}"
                    )
                    st.cache_data.clear()
