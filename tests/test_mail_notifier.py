...
class TestMailNotifier:
    ...
    def test_generate_csv(self, notifier):
        ...
        updates = [
            {"sku": "SKU1", "product_name": "Product 1", "old_price": 100, "new_price": 110, "status": "updated", "reason": ""},
            {"sku": "SKU2", "product_name": "Product 2", "old_price": 200, "new_price": None, "status": "error", "reason": "API error"},
        ]
    ...
    @patch.object(MailNotifier, 'send_message')
    @patch.object(MailNotifier, 'send_message_with_attachment')
    def test_send_detailed_report_with_csv_attachment(self, mock_with_attachment, mock_send, notifier):
        """Test send_detailed_report uses CSV attachment when over limit."""
        with patch('infrastructure.mail_notifier.settings') as mock_settings:
            mock_settings.NOTIFICATION_MAX_DETAILS = 2
            updates = [
                {"sku": f"SKU{i}", "product_name": f"Product {i}", "old_price": 100, "new_price": 110, "status": "updated", "reason": ""}
                for i in range(5)
            ]
