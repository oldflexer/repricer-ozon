...
from infrastructure.file_utils import wait_for_excel_available, save_safely, LOCK_WAIT_TIMEOUT
...
class TestFileUtils:
    ...
    def test_wait_for_excel_available_timeout(self):
        ...
        with patch("builtins.open", side_effect=PermissionError("File busy")), \
             patch("time.sleep"), \
    ...
    def test_save_safely_all_retries_fail(self):
        ...
        with patch("shutil.copy2"), \
             patch("infrastructure.file_utils.load_workbook", side_effect=Exception("Persistent error")), \
             patch("os.path.exists", return_value=True), \
             patch("os.remove"), \
             patch("time.sleep"):
